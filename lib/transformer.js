...
export const Transformer = {
    ...
    preprocessThreeDashes: function (content) {
        ...
        const contentBeforeTable = content.slice(0, indexOfFirst);
        const contentAfterTable = content.slice(indexOfSecond + 3); // +3 to skip the dashes

        const tableContent = content.slice(indexOfFirst + 3, indexOfSecond).trim();
        const keyValuePairs = tableContent.split("\n").map(row => {
            const [key, ...values] = row.trim().split(":").map(item => item.trim());
            const valuesColumn = values.join(":");
            return `| ${key} | ${valuesColumn} |`;
        }).join("\n");

        const markdownTable = `${contentBeforeTable}\n\n| Key | Values |\n| - | -- |\n${keyValuePairs}\n\r\n\n${contentAfterTable}`;
